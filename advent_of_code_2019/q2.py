...
The opcode indicates what to do; for example, 99 means that the
program is finished and should immediately halt. Encountering an
...
positions: position 9 contains 30, and position 10 contains 40.
Add these numbers together to get 70. Then, store this value at the
output position; here, the output position (3) is at position 3,
...
This opcode works just like the previous, but it multiplies
instead of adding. The inputs are at positions 3 and 11;
...
def test_process_intcode_program(f):
...
def process_intcode_program(program: str):
    codes = [int(code) for code in program.split(',')]
    return codes


print(process_intcode_program('1,2,3,4'))
